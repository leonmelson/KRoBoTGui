...
<html>
    ...
    if(isset($_GET['print']))
    {
		...
		?>
		<br>
		<form action="<?php FM_SELF_URL;?>" method="GET">
			<input type="hidden" name="p" value="<?php echo urlencode($_GET['p']);?>">
			<input type="hidden" name="VerifyPrint" value="<?php echo $PathToPrintFile;?>">
			<input id="VerifyPrint" type=submit value="VERIFY">
		</form>
		<?php
    }
    if(isset($_GET['VerifyPrint']))
    {
		foreach($get_help["robots"] as $robot => $r_value)
		{

			if($r_value['active'] and $r_value['primary'])
			{
				$send_to_robot = $robot;
			}
		}
		$url_to_file = $path_to_uploads.$_GET['VerifyPrint'];
		$params_url = ['url' => $url_to_file];
		$send_gcode_url = json_encode(['id' => '123', 'method' => 'send_gcode_url', 'api_key' => $api_key, 'robot' => $send_to_robot, 'params' => $params_url]);
        $VerifyPrintUrlDecode = urldecode($_GET['VerifyPrint']);
        // $KRoBoT->Push("File", $KRoBoT->FixPrintFileUrl($VerifyPrintUrlDecode), 0);
		print_r(curlPost($url, $send_gcode_url));
        echo "<br>File Sent to KRoBoT => $send_to_robot =".$VerifyPrintUrlDecode;
    }
    ?>
    </body>
</html>
                           ...
                           <?php if(strpos($f, "gcode") !== false)
                            {
                                ...
                                ?>
                            <a title="PRINT" href="?p=<?php echo urlencode(FM_PATH); ?>&amp;print=<?php echo urlencode($printfilepath); ?>"><i class="fa fa-android"></i></a>
                            <?php }?>
                            <a href="<?php echo $filelink ?>" title="File info"> <?php echo fm_convert_win($f) ?>
