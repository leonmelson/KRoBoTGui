...
$PathToUploads = "http://".$_SERVER['HTTP_HOST']."/KRoBoT/uploads/";
...
$default_timezone = 'Africa/Johannesburg'; // UTC
...
<html>
    ...
    if(isset($_GET['print']))
    {
        $PathToPrintFile = $PathToUploads.urldecode($_GET['print']);
    
        if($KRoBoT->Status=="Ready")
        {
            echo "Verify Printing of ".$PathToPrintFile;
            ?>
            <br>
            <a class="VerifyPrint" href="<?php FM_SELF_URL;?>?p=<?php echo urlencode($_GET['p']);?>&VerifyPrint=<?php echo urlencode($PathToPrintFile);?>">VERIFY</a>
            <?php
        }
        else
        {
        ...
        //$KRoBoT->Push("File", $PathToPrintFile);
        //echo "File Sent to KRoBoT => ".$f;
    }
    if(isset($_GET['VerifyPrint']))
    {
        $VerifyPrintUrlDecode = urldecode($_GET['VerifyPrint']);
        $KRoBoT->Push("File", $KRoBoT->FixPrintFileUrl($VerifyPrintUrlDecode), 0);
        echo "File Sent to KRoBoT =>".$VerifyPrintUrlDecode;
    }
    ?>
    </body>
</html>
...
if (isset($_GET['upload']) && !FM_READONLY) {
    ...
    fm_show_nav_path(FM_PATH, $KRoBoT->Name, $KRoBoT->Status); // current path
    ?>
                    ...
                    <td>
                        <div class="filename">
                            <?php if(strpos($f, "gcode") !== false)
                            {
                                ...
                                {
                                    $printfilepath = FM_PATH."/".$f; 
                                }
                                ?>
                            <a title="PRINT" href="?p=<?php echo urlencode(FM_PATH); ?>&amp;print=<?php echo urlencode($printfilepath); ?>"><i class="fa fa-android"></i> <?php echo fm_convert_win($f) ?></a>
                            <?php }?>
...
{
    return array(
        'txt', 'gcode', 'css', 'ini', 'conf', 'log', 'htaccess', 'passwd', 'ftpquota', 'sql', 'js', 'json', 'sh', 'config',
 ...
 * @param string $path
 */
function fm_show_nav_path($path, $KRoBoTName, $KRoBoTStatus)
{
